...
class ForemanInventory(object):
    ...
    def _resolve_params(self, host):
        ...
        params = {}
	host_id = host.get('id')
        for param in self._get_all_params_by_id(host_id):
            name = param['name']
	...
	# potential code to do async calls to satellite. Lets see if this reduces
	# the time
	pool = ThreadPoolExecutor()
	futures = [pool.submit(self._resolve_params, host) for host in self._get_hosts()]
        results = {}
	for r in as_completed(futures):
        ...
        for host in self._get_hosts():
            ...
            params = results.get(host.get('id'))
...
#    sys.exit(not ForemanInventory().run())
     cProfile.run('ForemanInventory().run()')
